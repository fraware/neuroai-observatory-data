...
def _programme()->dict[str,Any]:
    p=_load(PROGRAMME)
    if p.get("programme_id")!="SU-REGULATION-OPENFDA-UDI-v0.1":raise ValueError("Invalid UDI programme")
    return p

def _load_projector()->Projector:
    try:from neuroai_workbench.discovery import project_openfda_udi_pages
    except (ImportError,AttributeError) as exc:raise RuntimeError("Required Workbench capability project_openfda_udi_pages unavailable; integration remains PENDING_S1_MERGE") from exc
    return project_openfda_udi_pages

def _eligible(source:Mapping[str,Any])->bool:
    c=str(source.get("source_class") or "").upper();loc=str(source.get("canonical_locator") or "").upper()
    return "DEVICE" in c or "REGULATORY" in c or "UDI" in c or "GUDID" in loc

def _exact_udi_from_locator(locator:str)->set[str]:
    return {f"UDI:{m.group(1).upper()}:{m.group(2)}" for m in EXACT_UDI_RE.finditer(unquote(locator))}
...
def build_known_udi_source_index()->dict[str,Any]:
    ...
    for family,module in modules:
        for source in module.project().get("sources",[]):
            ...
            for identity in _exact_udi_from_locator(loc):
                prior=index.get(identity.upper())
                if prior and prior!=sid:raise ValueError(f"Conflicting controlled Sources for {identity}")
                index[identity.upper()]=sid
    if len(all_ids)!=248:raise ValueError(f"Expected 248 Sources, found {len(all_ids)}")
    return {"materialized_source_count":248,"udi_eligible_source_count":len(eligible),"known_exact_udi_identity_count":len(index),"udi_identity_to_source":dict(sorted(index.items())),"global_udi_completeness_claim":False}
...
def _validate_bundle(bundle:Mapping[str,Any],p:dict[str,Any])->tuple[str,list[dict[str,Any]],dict[str,dict[str,Any]]]:
    ...
    for c in captures:
        ...
        if qid not in configured or not isinstance(leaf,str) or not leaf or leaf in seen:raise ValueError("Invalid or duplicate UDI leaf")
        ...
        if part:
            if len(part)!=1 or part[0].get("dimension")!="PUBLIC_VERSION_DATE":raise ValueError(f"{leaf}: only PUBLIC_VERSION_DATE partition supported")
            lo,hi=part[0].get("lower_bound"),part[0].get("upper_bound");expected=f'({root})+AND+public_version_date:[{lo}+TO+{hi}]'
            if effective!=expected:raise ValueError(f"{leaf}: partitioned search must exactly bind public-version-date interval")
        pages=c.get("pages")
...
def build_replay(bundle:Mapping[str,Any],*,projector:Projector|None=None,known_index:Mapping[str,Any]|None=None)->dict[str,Any]:
    p=_programme();scope,captures,configured=_validate_bundle(bundle,p);projector=projector or _load_projector();known=dict(known_index) if known_index is not None else build_known_udi_source_index();known_map=known["udi_identity_to_source"]
    reports=[];union={};blocker_count=0;represented={c["query_id"] for c in captures};partitioned=sum(bool(c["partition_path"]) for c in captures);unresolved_total=0;multiple_total=0
    for c in sorted(captures,key=lambda x:x["leaf_query_id"]):
        proj=projector(query_id=c["query_id"],search=c["effective_search"],pages=c["pages"],known_udi_sources=known_map)
        cov=proj.get("coverage");records=proj.get("result_records");norms=proj.get("normalized_records")
        if not isinstance(cov,dict) or not isinstance(records,list) or not isinstance(norms,list):raise ValueError("Invalid Workbench UDI projection shape")
        blockers=_leaf_blockers(cov,p);blocker_count+=len(blockers);unresolved_total+=int(cov.get("unresolved_primary_di_count") or 0);multiple_total+=int(cov.get("multiple_primary_di_count") or 0)
        ...
        if len(norm_by)!=len(norms):raise ValueError("Missing/duplicate normalized UDI identity")
        for r in records:
            identity=r.get("record_key")
            if identity not in norm_by:raise ValueError("UDI candidate lacks normalized record")
            n=norm_by[identity];digest=n.get("normalized_record_sha256");candidate={x:r.get(x) for x in ("record_key","title","url","publisher","source_class","suggested_source_id","classification_hint","duplicate_of_source_id")}
            prior=union.get(identity)
            if prior:
                if prior["normalized_record_sha256"]!=digest or prior["candidate"]!=candidate:raise ValueError(f"Cross-query conflict for UDI identity {identity}")
    ...
    reconciliation={"capture_scope":scope,"materialized_source_namespace_count":known["materialized_source_count"],"known_exact_udi_identity_count":known["known_exact_udi_identity_count"],"configured_active_query_count":len(configured),"represented_query_count":len(represented),"all_logical_queries_represented":all_queries,"partitioned_leaf_count":partitioned,"partition_reconciliation_required":partitioned>0,"leaf_mechanical_blocker_count":blocker_count,"union_unique_udi_identity_count":len(union),"unresolved_primary_di_count":unresolved_total,"multiple_primary_di_count":multiple_total,"known_controlled_duplicate_count":len(known_dups),"new_candidate_input_count":len(new),"mechanically_complete":mechanically_complete,"automatic_di_relationship_creation":False,"automatic_premarket_authorization_relationship_creation":False,"automatic_current_commercial_availability_claim_creation":False,"automatic_marketing_authorization_claim_creation":False,"automatic_effectiveness_claim_creation":False,"automatic_system_conformance_claim_creation":False,"automatic_reopening_decision":False,"automatic_assessment_mutation":False,"canonical_successor_ready":False,"global_neuroai_udi_coverage_claim":False}
    return {"normalized_records":normalized,"known_duplicates":known_dups,"new_candidates":new,"query_reports":reports,"known_source_index_summary":{k:v for k,v in known.items() if k!="udi_identity_to_source"},"reconciliation":reconciliation,"input_provenance":{"programme_id":p["programme_id"],"provider":p["provider_contract"]["provider"],"captured_at":bundle.get("captured_at"),"input_sha256":_digest(bundle)}}

def write_projection(result:Mapping[str,Any],out:Path)->dict[str,str]:
    out.mkdir(parents=True,exist_ok=True);files={"normalized-udi-records.jsonl":b"".join(_canon(r) for r in result["normalized_records"]),"known-udi-duplicates.jsonl":b"".join(_canon(r) for r in result["known_duplicates"]),"new-udi-candidate-inputs.jsonl":b"".join(_canon(r) for r in result["new_candidates"]),"query-reports.json":_canon(result["query_reports"]),"reconciliation.json":_canon(result["reconciliation"]),"known-source-index-summary.json":_canon(result["known_source_index_summary"]),"input-provenance.json":_canon(result["input_provenance"])};manifest={}
    for name,data in files.items():(out/name).write_bytes(data);manifest[name]=hashlib.sha256(data).hexdigest()
    m=_canon({"files":dict(sorted(manifest.items())),"raw_openfda_pages_emitted":False,"customer_contacts_emitted":False,"labeler_duns_emitted":False,"canonical_successor_ready":False});(out/"manifest.json").write_bytes(m);manifest["manifest.json"]=hashlib.sha256(m).hexdigest();return manifest
