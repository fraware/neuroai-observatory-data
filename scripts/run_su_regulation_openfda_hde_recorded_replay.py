...
HDE_LOCATOR_RE=re.compile(r"(?:pma_number|hde_number|hde)(?:=|:|/|%3A|%3D)%?22?(H[A-Za-z0-9._-]+)",re.I);SUPP_RE=re.compile(r"(?:supplement_number|supplement|supp)(?:=|:|/|%3A|%3D)%?22?([A-Za-z0-9._-]+)",re.I)
...
def _load_projector()->Projector:
    ...
    except (ImportError,AttributeError) as exc:raise RuntimeError("Required Workbench capability project_openfda_hde_pages unavailable; integration remains PENDING_S1_MERGE") from exc
...
def _composite_from_locator(locator:str)->set[str]:
    text=unquote(locator);hdes={m.group(1).upper() for m in HDE_LOCATOR_RE.finditer(text)};supps={m.group(1).upper() for m in SUPP_RE.finditer(text)}
    if len(hdes)!=1:return set()
    h=next(iter(hdes))
    if len(supps)==1:return {f"HDE:{h}:{next(iter(supps))}"}
    explicit_original=any(t in text.lower() for t in ("original_application","original-application","record_role=original","record_role:original"));return {f"HDE:{h}:ORIGINAL"} if explicit_original else set()
def build_known_record_source_index()->dict[str,Any]:
    modules=(("V1_4",project_v14_sources_to_v2),("V1_6",project_v16_sources_observations_to_v2),("V1_7",project_v17_prima_sources_to_v2));all_ids=set();eligible=set();idx={}
    for family,module in modules:
        for s in module.project().get("sources",[]):
            ...
            for identity in _composite_from_locator(loc):
                ...
                if prior and prior!=sid:raise ValueError(f"Conflicting controlled Sources for HDE record {identity}")
                idx[identity]=sid
    if len(all_ids)!=248:raise ValueError(f"Expected 248 Sources, found {len(all_ids)}")
    return {"materialized_source_count":248,"regulatory_typed_source_count":len(eligible),"known_composite_hde_record_count":len(idx),"record_to_source":dict(sorted(idx.items())),"global_hde_completeness_claim":False}
...
def _validate_bundle(bundle:Mapping[str,Any],p:dict[str,Any])->tuple[str,list[dict[str,Any]],dict[str,dict[str,Any]]]:
    ...
    for c in captures:
        ...
        if not isinstance(effective,str) or not isinstance(part,list):raise ValueError(f"{leaf}: invalid search/partition")
        ...
        if part:
            ...
            if effective!=expected or not (isinstance(lo,str) and isinstance(hi,str) and re.fullmatch(r"\d{8}",lo) and re.fullmatch(r"\d{8}",hi) and lo<=hi):raise ValueError(f"{leaf}: invalid decision-date partition")
        pages=c.get("pages")
...
def _leaf_blockers(cov:Mapping[str,Any],p:Mapping[str,Any])->list[str]:
    b=[];missing=set(p["coverage_contract"]["required_metrics_per_leaf_query"])-set(cov)
    if missing:b.append("MISSING_METRICS:"+",".join(sorted(missing)))
    for k,v in p["coverage_contract"]["mechanical_completion_requires"].items():
        if cov.get(k)!=v:b.append(f"GATE:{k}:{cov.get(k)!r}!={v!r}")
    for k in ("decision_semantics_derived_only_from_exact_decision_code",):
        if cov.get(k) is not True:b.append("DECISION_SEMANTICS_BOUNDARY_MISSING")
    for k in ("hde_approval_is_reasonable_assurance_effectiveness_claim","hde_approval_establishes_facility_irb_approval"):
        if cov.get(k) is not False:b.append(f"AUTHORITY_BOUNDARY:{k}")
    return b
def build_replay(bundle:Mapping[str,Any],*,projector:Projector|None=None,known_index:Mapping[str,Any]|None=None)->dict[str,Any]:
    ...
    reports=[];union={};blockers=0;represented={c["query_id"] for c in captures};partitioned=sum(bool(c["partition_path"]) for c in captures);non_h=unresolved=0
    for c in sorted(captures,key=lambda x:x["leaf_query_id"]):
        proj=projector(query_id=c["query_id"],search=c["effective_search"],pages=c["pages"],known_record_sources=known_map);cov=proj.get("coverage");records=proj.get("result_records");norms=proj.get("normalized_records")
        if not isinstance(cov,dict) or not isinstance(records,list) or not isinstance(norms,list):raise ValueError("Invalid Workbench HDE projection shape")
        lb=_leaf_blockers(cov,p);blockers+=len(lb);non_h+=int(cov.get("out_of_scope_non_h_prefix_count") or 0);unresolved+=int(cov.get("unresolved_hde_number_count") or 0);reports.append({"query_id":c["query_id"],"leaf_query_id":c["leaf_query_id"],"partition_path":c["partition_path"],"effective_search":c["effective_search"],"capture_sha256":_digest(c),"coverage":cov,"mechanical_blockers":lb})
        norm_by={n.get("record_identity"):n for n in norms if isinstance(n,dict)}
        if len(norm_by)!=len(norms):raise ValueError("Missing/duplicate normalized HDE identity")
        for r in records:
            identity=r.get("record_key")
            if identity not in norm_by:raise ValueError("HDE candidate lacks normalized record")
            n=norm_by[identity];digest=n.get("normalized_record_sha256");candidate={x:r.get(x) for x in ("record_key","title","url","publisher","source_class","suggested_source_id","classification_hint","duplicate_of_source_id","decision_semantics")};prior=union.get(identity)
            if prior:
                if prior["normalized_record_sha256"]!=digest or prior["candidate"]!=candidate:raise ValueError(f"Cross-query conflict for HDE record {identity}")
    ...
    reconciliation={"capture_scope":scope,"materialized_source_namespace_count":known["materialized_source_count"],"known_composite_hde_record_count":known["known_composite_hde_record_count"],"configured_active_query_count":len(configured),"represented_query_count":len(represented),"all_logical_queries_represented":all_queries,"partitioned_leaf_count":partitioned,"partition_reconciliation_required":partitioned>0,"leaf_mechanical_blocker_count":blockers,"union_unique_composite_record_count":len(union),"out_of_scope_non_h_prefix_count":non_h,"unresolved_hde_number_count":unresolved,"known_controlled_duplicate_count":len(known_dups),"new_candidate_input_count":len(new),"mechanically_complete":complete,"exact_decision_code_map":p["decision_semantics_policy"]["exact_code_map"],"hde_approval_state_requires_exact_appr_code":True,"hde_standard_uses_probable_benefit_risk_boundary":True,"hde_approval_is_reasonable_assurance_effectiveness_claim":False,"hde_approval_establishes_facility_irb_approval":False,"automatic_original_supplement_lineage_relationship_creation":False,"automatic_device_or_applicant_entity_creation":False,"automatic_effectiveness_claim_creation":False,"automatic_facility_irb_authorization_claim_creation":False,"automatic_current_commercial_configuration_claim_creation":False,"automatic_global_authorization_claim_creation":False,"automatic_system_conformance_claim_creation":False,"automatic_reopening_decision":False,"automatic_assessment_mutation":False,"canonical_successor_ready":False,"global_neuroai_hde_coverage_claim":False}
...
def write_projection(result:Mapping[str,Any],out:Path)->dict[str,str]:
    out.mkdir(parents=True,exist_ok=True);files={"normalized-hde-records.jsonl":b"".join(_canon(r) for r in result["normalized_records"]),"known-hde-duplicates.jsonl":b"".join(_canon(r) for r in result["known_duplicates"]),"new-hde-candidate-inputs.jsonl":b"".join(_canon(r) for r in result["new_candidates"]),"query-reports.json":_canon(result["query_reports"]),"reconciliation.json":_canon(result["reconciliation"]),"known-source-index-summary.json":_canon(result["known_source_index_summary"]),"input-provenance.json":_canon(result["input_provenance"])};manifest={}
