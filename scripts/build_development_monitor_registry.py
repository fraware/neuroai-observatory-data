...
def build_development_registry(
    ...
    if len(candidate_records) != 2:
        raise ValueError(f"Expected exactly two active recurring extension candidates, got {len(candidate_records)}")
    ...
    if set(lifecycle_source_ids) & set(source_ids):
        raise ValueError("Lifecycle-resolved source leaked into active development monitor registry")
...
def verify_development_registry(registry: dict[str, Any]) -> None:
    ...
    if metadata.get("record_count") != 226 or len(sources) != 226:
        raise ValueError(f"Development registry must contain exactly 226 active monitors, got {len(sources)}")
    if metadata.get("predecessor_record_count") != 224 or metadata.get("extension_record_count") != 2:
    ...
    if metadata.get("lifecycle_resolved_source_count") != 1:
        raise ValueError("Development registry must bind exactly one lifecycle-resolved source")
        ...
        json.dumps(
            {
                ...
                "lifecycle_resolved_source_ids": registry["metadata"]["lifecycle_resolved_source_ids"],
