#!/usr/bin/env python3
"""Reconcile dated EPO OPS partition leaves against exact bounded parent denominators.

The proof is intentionally bounded to publication dates from 1800-01-01 through the replay
capture date. It does not establish completeness for undated/unusable-date records or for the
...
REQUIRED_REPLAY_FILES = {
    ...
    "known-patent-duplicates.jsonl",
    "new-patent-candidate-inputs.jsonl",
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        if line.strip():
            ...
            if not isinstance(value, dict):
                raise ValueError(f"{path.name}: JSONL row must be object")
            rows.append(value)
    return rows
...
def _programme() -> dict[str, Any]:
    ...
    if value.get("programme_id") != "SU-PATENTS-EPO-OPS-v0.1":
        raise ValueError("Invalid SU-PATENTS EPO OPS programme")
    dependency = value.get("workbench_dependency") or {}
    if dependency.get("integration_state") != "AVAILABLE":
        raise ValueError("EPO OPS partition proof requires AVAILABLE Workbench capability")
    return value
...
def _root_key(query_id: Any, applicant_term: Any) -> tuple[str, str | None]:
    if not isinstance(query_id, str) or not query_id:
        raise ValueError("partition root/query report missing query_id")
    if applicant_term is not None and not isinstance(applicant_term, str):
        raise ValueError("applicant_term must be string/null")
    return query_id, applicant_term
...
def load_replay_output(replay_output_dir: Path) -> dict[str, Any]:
    """Load and verify one exact replay package before using it as proof input."""
    manifest_path = replay_output_dir / "manifest.json"
    if not manifest_path.is_file():
        raise ValueError("Replay output directory has no manifest.json")
    manifest = _load(manifest_path)
    if not isinstance(manifest, dict):
        raise ValueError("Replay manifest must be object")
    if manifest.get("programme_id") != "SU-PATENTS-EPO-OPS-v0.1":
        raise ValueError("Replay manifest programme_id mismatch")
    if manifest.get("raw_ops_xml_emitted") is not False:
        raise ValueError("Replay manifest cannot emit raw OPS XML")
    ...
    if not isinstance(entries, list):
        raise ValueError("Replay manifest files must be array")
    by_path: dict[str, Mapping[str, Any]] = {}
    for entry in entries:
        if not isinstance(entry, Mapping):
            raise ValueError("Replay manifest file entry must be object")
        name = entry.get("path")
        if not isinstance(name, str) or not name or Path(name).name != name:
            raise ValueError("Replay manifest paths must be plain filenames")
        if name in by_path:
            raise ValueError(f"Duplicate replay manifest path {name}")
        by_path[name] = entry
    if set(by_path) != REQUIRED_REPLAY_FILES:
        raise ValueError("Replay manifest file set changed")
    if manifest.get("file_count") != len(REQUIRED_REPLAY_FILES):
        raise ValueError("Replay manifest file_count mismatch")

    for name, entry in by_path.items():
        path = replay_output_dir / name
        if not path.is_file():
            raise ValueError(f"Replay manifest file missing: {name}")
        payload = path.read_bytes()
        if entry.get("bytes") != len(payload):
            raise ValueError(f"Replay file byte-count mismatch: {name}")
    ...
    provenance = _load(replay_output_dir / "input-provenance.json")
    source_summary = _load(replay_output_dir / "known-source-index-summary.json")
    reconciliation = _load(replay_output_dir / "reconciliation.json")
    reports = _load(replay_output_dir / "query-reports.json")
    normalized = _load_jsonl(replay_output_dir / "normalized-patents.jsonl")

    digests = {
        provenance.get("controlled_source_id_set_sha256"),
        source_summary.get("source_id_set_sha256"),
        reconciliation.get("controlled_source_id_set_sha256"),
    }
    if len(digests) != 1:
        raise ValueError("Replay controlled Source digest is internally inconsistent")
    source_digest = next(iter(digests))
    if not isinstance(source_digest, str) or len(source_digest) != 64:
        raise ValueError("Replay controlled Source digest missing/invalid")
    if (
        source_summary.get("materialized_source_count") != 248
        or reconciliation.get("materialized_source_namespace_count") != 248
    ):
        raise ValueError("Replay must bind the exact 248-Source namespace")

    return {
        "manifest_sha256": _manifest_sha256(manifest_path),
        "manifest": manifest,
        "input_provenance": provenance,
        "known_source_index_summary": source_summary,
...
def _validate_proof_bundle(
    proof: Mapping[str, Any], replay: Mapping[str, Any]
    ...
    if _parse_date(upper, "capture date") < _parse_date(LOWER_BOUND, "lower bound"):
        raise ValueError("Replay capture date predates dated-universe lower bound")
    ...
    for row in reports:
        ...
        if not isinstance(leaf, str) or not leaf or leaf in by_leaf:
            raise ValueError("Replay leaf_query_id missing or duplicated")
        by_leaf[leaf] = row
    ...
    *,
    ...
    for row in intervals:
        if not isinstance(row, dict) or set(row) != {
            ...
            raise ValueError("leaf interval fields changed")
        leaf_id = row.get("leaf_query_id")
        if not isinstance(leaf_id, str) or not leaf_id or leaf_id in declared_leaf_ids:
            raise ValueError("leaf interval ID missing or duplicated")
        declared_leaf_ids.add(leaf_id)
    ...
    for previous, current in zip(parsed, parsed[1:]):
        ...
        if current[0] != expected:
            blockers.append(
                "PARTITION_INTERVAL_OVERLAP"
                if current[0] <= previous[1]
                else "PARTITION_INTERVAL_GAP"
            )
    ...
    for lower_date, upper_date, row in parsed:
        ...
        if replay_report is None:
            ...
            continue
        if replay_report.get("query_id") != query_id:
            blockers.append(f"LEAF_QUERY_ROOT_MISMATCH:{leaf_id}")
        if replay_report.get("applicant_term") != applicant_term:
            blockers.append(f"LEAF_APPLICANT_ROOT_MISMATCH:{leaf_id}")
        if not replay_report.get("partition_path"):
            blockers.append(f"LEAF_IS_NOT_PARTITIONED:{leaf_id}")

        expected_cql = _bounded_cql(
            root_cql,
            lower_date.strftime("%Y%m%d"),
            upper_date.strftime("%Y%m%d"),
        )
        if replay_report.get("effective_cql") != expected_cql:
            blockers.append(f"LEAF_CQL_MISMATCH:{leaf_id}")
        if replay_report.get("mechanical_blockers") != []:
        ...
        for key, expected in (
            ...
            if coverage.get(key) != expected:
                blockers.append(f"LEAF_COVERAGE_GATE:{leaf_id}:{key}")
    ...
    return interval_reports, blockers, declared_leaf_ids


def _cross_partition_duplicates(
    normalized_patents: Any, leaf_ids: set[str]
) -> list[dict[str, Any]]:
    ...
    for row in normalized_patents:
        ...
        if len(overlap) > 1:
            duplicates.append(
                {
                    "docdb_publication_reference": row.get(
...
def build_partition_proof(
    ...
    *,
    ...
    if reconciliation.get("mechanically_complete") is True:
        replay_blockers.append("REPLAY_IMPROPERLY_MARKED_COMPLETE")
    ...
    replay_partitioned_root_keys: set[tuple[str, str | None]] = set()
    replay_partitioned_leaf_ids: set[str] = set()
    for leaf_id, report in by_leaf.items():
        if report.get("partition_path"):
            replay_partitioned_leaf_ids.add(leaf_id)
            replay_partitioned_root_keys.add(
                _root_key(report.get("query_id"), report.get("applicant_term"))
            )

    proof_root_keys: set[tuple[str, str | None]] = set()
    ...
    for root in roots:
        ...
        applicant_term = root.get("applicant_term")
        root_key = _root_key(query_id, applicant_term)
        if root_key in proof_root_keys:
            raise ValueError(f"Duplicate partition proof root {root_key}")
        proof_root_keys.add(root_key)
        ...
        if parent_coverage.get("range_sequence_valid") is not True:
            blockers.append("PARENT_RANGE_SEQUENCE_INVALID")
        parent_total = parent_coverage.get("reported_total_result_count")
        if (
            not isinstance(parent_total, int)
            or isinstance(parent_total, bool)
            or parent_total <= 2000
        ):
            blockers.append("PARENT_NOT_OVER_RETRIEVAL_LIMIT")
        if (
            parent_coverage.get("over_2000_limit") is not True
            or parent_coverage.get("partition_required") is not True
            or parent_coverage.get("candidate_emission_refused_due_to_over_limit")
            is not True
        ):
            blockers.append("PARENT_DID_NOT_FAIL_CLOSED_OVER_LIMIT")
        ...
        intervals, interval_blockers, declared_leaf_ids = _interval_proof(
            root=root,
            root_cql=root_cql,
        ...
        collision = declared_leaf_ids & used_leaf_ids
        if collision:
            blockers.append(
                "LEAF_ASSIGNED_TO_MULTIPLE_ROOTS:" + ",".join(sorted(collision))
            )
        used_leaf_ids.update(declared_leaf_ids)
        ...
        duplicates = _cross_partition_duplicates(
            normalized_patents, declared_leaf_ids
        )
        if duplicates:
    ...
    missing_root_keys = replay_partitioned_root_keys - proof_root_keys
    extra_root_keys = proof_root_keys - replay_partitioned_root_keys
    if missing_root_keys:
        replay_blockers.append("PARTITIONED_REPLAY_ROOTS_MISSING_FROM_PROOF")
    if extra_root_keys:
        replay_blockers.append("PROOF_ROOTS_NOT_PRESENT_IN_REPLAY")
    ...
    return {
        ...
        "controlled_source_id_set_sha256": replay.get(
            "controlled_source_id_set_sha256"
        )
        or (replay.get("input_provenance") or {}).get(
            "controlled_source_id_set_sha256"
        ),
        "dated_universe_lower_bound": LOWER_BOUND,
        "dated_universe_upper_bound": upper_bound,
        ...
        "authority_boundary": (
            "A complete proof establishes only exact denominator reconciliation for explicitly "
            "dated OPS search universes from 1800-01-01 through the replay capture date. It "
            "excludes records without usable publication-date indexing and does not establish "
            "unbounded provider completeness, global NeuroAI patent recall, family identity, "
            "ownership, implementation, validity/enforceability, freedom to operate, system "
            "capability, safety/effectiveness, assessment effect, or canonical authority."
        ),
    }
...
def write_proof(result: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "dated_partition_reconciliation_complete": result[
            "dated_partition_reconciliation_complete"
        ],
        "canonical_successor_ready": False,
    }
