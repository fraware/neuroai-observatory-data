"""Validate current bounded openFDA device-recall discovery without network access."""
...
def validate_programme(p:dict[str,Any],registry:dict[str,Any])->dict[str,Any]:
    ...
    require_source_universe_stream(registry,"SU-SAFETY-US")
    ...
    dep=p.get("workbench_dependency") or {}
    _require(dep=={"minimum_package_line":"0.3.0.dev0","required_capability":"project_openfda_device_recall_pages","integration_state":"AVAILABLE"},"Current merged recall capability must remain AVAILABLE")
    identity=p.get("identity_policy") or {};_require(identity.get("primary_identity")=="CFRES_ID","Recall identity changed")
    ...
    for key in ("same_event_number_auto_merge","k_number_auto_system_merge","pma_number_auto_system_merge","product_code_auto_system_merge","recalling_firm_auto_entity_merge"):_require(identity.get(key) is False,f"{key} must remain false")
    _require(identity.get("conflicting_same_cfres_id_policy")=="FAIL_CLOSED","cfres_id conflicts must fail closed")
    inclusion=p.get("inclusion_policy") or {}
    for key in ("automatic_source_admission","automatic_recall_event_entity_creation","automatic_system_or_device_entity_creation","automatic_firm_entity_creation","automatic_submission_relationship_creation","automatic_system_nonconformance_claim_creation","automatic_assessment_mutation","automatic_reopening_decision","automatic_monitor_creation"):_require(inclusion.get(key) is False,f"{key} must remain false")
    _require(inclusion.get("human_relevance_review_required") is True,"Human recall review required")
    paging=p.get("paging_policy") or {}
    _require(paging.get("execution_mode_v0_1")=="SKIP_LIMIT_BOUNDED","Recall paging mode changed")
    _require((paging.get("page_limit"),paging.get("initial_skip"),paging.get("max_skip"),paging.get("max_direct_result_count"))==(1000,0,25000,26000),"Recall paging bounds changed")
    _require(paging.get("preferred_partition_field")=="event_date_posted","Recall partition field changed")
    for key in ("silent_truncation_allowed","partial_over_limit_candidate_emission_allowed"):_require(paging.get(key) is False,f"{key} must remain false")
    _require(paging.get("search_after_not_yet_authorized_by_v0_1_projector") is True,"v0.1 must not silently enable search-after")
    _require(paging.get("partition_provenance_required") is True,"Partition provenance required")
    streams=p.get("query_streams");_require(isinstance(streams,list),"query_streams must be list")
    ids=[str(row.get("query_id") or "") for row in streams];_require(len(ids)==len(set(ids)) and set(ids)==EXPECTED_QUERY_IDS,"Recall query stream set changed")
    for row in streams:
        qid=row["query_id"];_require(row.get("cadence")=="WEEKLY" and row.get("status")=="ACTIVE",f"{qid}: cadence/status changed")
        _require(row.get("completeness_claim") is False,f"{qid}: cannot claim completeness")
        search=row.get("search");_require(isinstance(search,str) and "product_description:" in search and "+OR+" in search,f"{qid}: explicit product-description OR search required")
    projection=p.get("candidate_projection") or {};_require(set(projection.get("required_fields") or [])==REQUIRED_FIELDS,"Recall candidate fields changed")
    for key in ("address_or_contact_fields_in_discovery_layer","code_info_lot_serial_text_in_discovery_layer","distribution_pattern_in_discovery_layer","raw_api_pages_emitted_to_s2"):_require(projection.get(key) is False,f"{key} must remain false")
    coverage=p.get("coverage_contract") or {};_require(set(coverage.get("required_metrics_per_leaf_query") or [])==REQUIRED_COVERAGE,"Recall coverage metrics changed")
    ...
    for key in ("device_recall_database_completeness_claim","global_neuroai_device_recall_claim","query_recall_claim","recall_status_is_complete_lifecycle_tracker","recall_record_establishes_broader_system_nonconformance"):_require(coverage.get(key) is False,f"{key} must remain false")
    review=p.get("human_review_contract") or {}
    for key in ("automatic_acceptance","recall_record_is_global_product_unsafety_evidence","recall_record_is_all_configuration_nonconformance_evidence","recall_status_is_authoritative_live_lifecycle_state","recall_record_automatically_reopens_assessment"):_require(review.get(key) is False,f"{key} must remain false")
    network=p.get("network_policy") or {};_require(network=={"live_execution":"OPT_IN_ONLY","requires_workbench_network_gate":True,"configuration_performs_http":False,"hosted_success_claim_allowed_without_runner_steps":False},"Recall network boundary changed")
    return {"programme_id":p["programme_id"],"query_stream_count":len(streams),"integration_state":"AVAILABLE","network_requests_performed":False,"automatic_reopening":False,"canonical_mutation_performed":False}
