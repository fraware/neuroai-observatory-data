...
def validate_programme(p:dict[str,Any],reg:dict[str,Any])->dict[str,Any]:
    ...
    dep=p.get("workbench_dependency") or {}; _req(dep.get("required_capability")=="project_openfda_510k_pages","Unexpected Workbench capability"); _req(dep.get("integration_state") in {"NOT_IMPLEMENTED","PENDING_S1_MERGE"},"Unmerged 510(k) projector cannot be AVAILABLE")
    ...
    return {"programme_id":p["programme_id"],"query_stream_count":len(streams),"integration_state":dep["integration_state"],"den_out_of_scope":True,"record_presence_is_clearance_claim":False,"network_requests_performed":False,"canonical_mutation_performed":False}
