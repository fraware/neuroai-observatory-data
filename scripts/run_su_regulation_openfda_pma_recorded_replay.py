...
PMA_LOCATOR_RE=re.compile(r'(?:pma_number|pma)\s*(?:=|:|/)\s*"?((?:BP|P|D)[A-Za-z0-9._-]+)"?',re.I)
SUPP_LOCATOR_RE=re.compile(r'(?:supplement_number|supplement|supp)\s*(?:=|:|/)\s*"?([A-Za-z0-9._-]+)"?',re.I)
PMA_ID_RE=re.compile(r'^(?:BP|P|D)[A-Z0-9._-]+$',re.I)
Projector=Callable[...,dict[str,Any]]
ALLOWED_NORMALIZED_FIELDS={
    "record_kind","pma_number","supplement_number","record_identity","record_role","trade_name","generic_name",
    "applicant","date_received","decision_date","decision_code","decision_semantics","decision_code_recognized",
    "decision_supports_approval","product_code","supplement_type","supplement_reason","ao_statement",
    "expedited_review_flag","query_memberships","boundary","normalized_record_sha256",
}
...
def _programme()->dict[str,Any]:
    ...
    if dep.get("required_capability")!="project_openfda_pma_pages" or dep.get("integration_state")!="AVAILABLE":raise ValueError("Current merged PMA capability unavailable")
    return p

def _load_projector()->Projector:
    try:from neuroai_workbench.discovery import project_openfda_pma_pages
    except (ImportError,AttributeError) as exc:raise RuntimeError("Required Workbench PMA projector unavailable") from exc
    return project_openfda_pma_pages

def _eligible(source:Mapping[str,Any])->bool:
    token=str(source.get("source_class") or "").upper();return "REGULATORY" in token or "PMA" in token or "APPROVAL" in token

def _composite_from_locator(locator:str)->set[str]:
    text=unquote_plus(locator);pmas={m.group(1).upper() for m in PMA_LOCATOR_RE.finditer(text)};supps={m.group(1).upper() for m in SUPP_LOCATOR_RE.finditer(text)}
    if len(pmas)!=1:return set()
    pma=next(iter(pmas))
    if not PMA_ID_RE.fullmatch(pma):return set()
    if len(supps)==1:return {f"PMA:{pma}:{next(iter(supps))}"}
    lower=text.lower();explicit_original=any(token in lower for token in ("original_application","original-application","record_role=original","record_role:original","record_role=original_application","record_role:original_application"))
...
def build_known_record_source_index()->dict[str,Any]:
    ...
    for source in sources:
        ...
        for identity in sorted(_composite_from_locator(locator)):
            prior=index.get(identity)
            if prior is not None and prior!=sid:raise ValueError(f"Conflicting controlled Sources for PMA record {identity}")
            index[identity]=sid;lineage[identity]={"source_id":sid,"lineage_family":str(source.get("lineage_family") or "")}
    return {"materialized_source_count":248,"source_id_set_sha256":source_digest,"regulatory_typed_source_count":len(eligible),"known_composite_pma_record_count":len(index),"record_to_source":dict(sorted(index.items())),"record_lineage":{k:lineage[k] for k in sorted(lineage)},"global_pma_completeness_claim":False}
...
def _validate_bundle(bundle:Mapping[str,Any],p:dict[str,Any])->tuple[str,list[dict[str,Any]],dict[str,dict[str,Any]]]:
    ...
    seen:set[str]=set()
    for c in captures:
        if not isinstance(c,dict):raise ValueError("PMA leaf must be object")
...
def _leaf_blockers(cov:Mapping[str,Any],p:Mapping[str,Any])->list[str]:
    ...
    if cov.get("record_presence_is_approval_claim") is not False:blockers.append("RECORD_PRESENCE_APPROVAL_BOUNDARY_VIOLATION")
    return blockers
...
def _expected_decision(code:Any,p:Mapping[str,Any])->tuple[str,bool,bool]:
    ...
    if normalized in mapping:return mapping[normalized],True,normalized=="APPR"
...
def _validate_normalized(row:Any,leaf:str,p:Mapping[str,Any])->dict[str,Any]:
    if not isinstance(row,dict):raise ValueError(f"{leaf}: normalized PMA record must be object")
    pma=str(row.get("pma_number") or "").upper();supp=str(row.get("supplement_number") or "").upper();identity=row.get("record_identity");role=row.get("record_role")
    if not PMA_ID_RE.fullmatch(pma):raise ValueError(f"{leaf}: normalized PMA identity must use P/BP/D prefix")
    if not supp:raise ValueError(f"{leaf}/{pma}: supplement_number must be explicit ORIGINAL or supplement")
    expected_identity=f"PMA:{pma}:{supp}"
    if identity!=expected_identity:raise ValueError(f"{leaf}/{pma}: composite PMA record_identity mismatch")
    ...
    if extra:raise ValueError(f"{leaf}/{identity}: normalized PMA contains prohibited/unexpected fields: {','.join(sorted(extra))}")
    sem,recognized,approval=_expected_decision(row.get("decision_code"),p);actual=(row.get("decision_semantics"),row.get("decision_code_recognized"),row.get("decision_supports_approval"))
    if actual!=(sem,recognized,approval):raise ValueError(f"{leaf}/{identity}: PMA decision semantics do not match exact FDA decision code")
...
def build_replay(bundle:Mapping[str,Any],*,projector:Projector|None=None,known_index:Mapping[str,Any]|None=None)->dict[str,Any]:
    ...
    reports=[];union:dict[str,dict[str,Any]]={};blocker_count=0;raw_pages=0;hde_total=0;nda_total=0;unresolved_total=0
    ...
    for c in sorted(captures,key=lambda x:str(x["leaf_query_id"])):
        ...
        proj=projector(query_id=str(c["query_id"]),search=str(c["effective_search"]),pages=c["pages"],known_record_sources=known_map)
        cov=proj.get("coverage");records=proj.get("result_records");norms=proj.get("normalized_records")
        if not isinstance(cov,dict) or not isinstance(records,list) or not isinstance(norms,list):raise ValueError(f"{leaf}: invalid Workbench PMA projection")
        blockers=_leaf_blockers(cov,p);blocker_count+=len(blockers);hde_total+=int(cov.get("out_of_scope_hde_count") or 0);nda_total+=int(cov.get("out_of_scope_legacy_nda_count") or 0);unresolved_total+=int(cov.get("unresolved_pma_number_count") or 0)
        ...
        for raw in norms:
            ...
            if identity in norm_by:raise ValueError(f"{leaf}: duplicate normalized PMA record {identity}")
            norm_by[identity]=n
        for r in records:
            identity=r.get("record_key") if isinstance(r,dict) else None
            if not isinstance(identity,str) or identity not in norm_by:raise ValueError(f"{leaf}: PMA candidate lacks normalized record")
            ...
            if prior is None:union[identity]={"normalized":n,"digest":digest,"candidate":candidate,"queries":{str(c["query_id"])},"leaves":{leaf}}
                ...
                if prior["digest"]!=digest or prior["candidate"]!=candidate:raise ValueError(f"Cross-leaf conflict for PMA record {identity}")
                prior["queries"].add(str(c["query_id"]));prior["leaves"].add(leaf)
    normalized=[];known_dups=[];new=[]
    for identity in sorted(union):
        row=union[identity];n=dict(row["normalized"]);n["query_memberships"]=sorted(row["queries"]);n["leaf_memberships"]=sorted(row["leaves"]);normalized.append(n)
        ...
        else:raise ValueError(f"Unexpected PMA classification for {identity}")
    ...
    reconciliation={"scope":scope,"materialized_source_namespace_count":248,"controlled_source_id_set_sha256":source_digest,"known_composite_pma_record_count_before_run":known.get("known_composite_pma_record_count"),"configured_active_query_count":len(configured),"represented_query_count":len(represented),"all_logical_queries_represented":all_queries,"partitioned_leaf_count":partitioned,"partition_reconciliation_required":partitioned>0,"leaf_mechanical_blocker_count":blocker_count,"union_unique_composite_record_count":len(union),"out_of_scope_hde_count":hde_total,"out_of_scope_legacy_nda_count":nda_total,"unresolved_pma_number_count":unresolved_total,"known_controlled_duplicate_count":len(known_dups),"new_candidate_input_count":len(new),"raw_input_page_count":raw_pages,"raw_openfda_pages_emitted":False,"mechanically_complete":complete,"exact_decision_code_map":p["decision_semantics_policy"]["exact_code_map"],"record_presence_is_approval_claim":False,"supplement_approval_is_original_application_approval_claim":False,"automatic_original_supplement_lineage_relationship_creation":False,"automatic_device_or_applicant_entity_creation":False,"automatic_current_commercial_configuration_claim_creation":False,"automatic_global_authorization_claim_creation":False,"automatic_system_conformance_claim_creation":False,"automatic_reopening_decision":False,"automatic_assessment_mutation":False,"global_neuroai_pma_coverage_claim":False,"canonical_successor_ready":False}
    return {"schema_version":"0.1.0","status":"NONCANONICAL_SU_REGULATION_PMA_RECORDED_REPLAY_PROJECTION","programme_id":p["programme_id"],"normalized_pma_records":normalized,"known_duplicates":known_dups,"new_candidate_inputs":new,"query_reports":reports,"known_source_index_summary":{"materialized_source_count":248,"source_id_set_sha256":source_digest,"regulatory_typed_source_count":known.get("regulatory_typed_source_count"),"known_composite_pma_record_count":known.get("known_composite_pma_record_count"),"global_pma_completeness_claim":False},"reconciliation":reconciliation,"input_provenance":{"bundle_sha256":_digest(bundle),"captured_at":bundle["captured_at"],"controlled_source_id_set_sha256":source_digest,"known_record_index_sha256":_digest(known_map),"raw_provider_pages_retained_in_output":False}}
...
def write_projection(result:Mapping[str,Any],out:Path)->dict[str,Any]:
    ...
    for key,name in (("normalized_pma_records","normalized-pma-records.jsonl"),("known_duplicates","known-pma-duplicates.jsonl"),("new_candidate_inputs","new-pma-candidate-inputs.jsonl")):files.append(_write_jsonl(out/name,list(result[key])))
    ...
    manifest={"programme_id":result["programme_id"],"status":result["status"],"files":sorted(files,key=lambda x:x["path"]),"file_count":len(files),"raw_openfda_pages_emitted":False,"hde_records_emitted_as_pma_candidates":False,"legacy_nda_records_emitted_as_pma_candidates":False,"record_presence_is_approval_claim":False,"canonical_successor_ready":False};payload=_canon(manifest);(out/"manifest.json").write_bytes(payload);return {**manifest,"manifest_sha256":hashlib.sha256(payload).hexdigest()}
def main()->int:
