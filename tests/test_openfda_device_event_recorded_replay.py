...
class OpenFdaDeviceEventRecordedReplayTests(unittest.TestCase):
    ...
    def test_cross_query_same_report_unions_memberships(self) -> None:
        ...
        self.assertEqual(result["normalized_reports"][0]["query_memberships"], sorted([s["query_id"] for s in streams]))
    ...
    def test_partitioned_leaf_requires_separate_reconciliation(self) -> None:
        stream = replay._programme()["query_streams"][0]
        result = replay.build_replay(
            _bundle([_capture(stream, partition=("20260101", "20261231"))]),
    ...
    def test_partition_query_must_exactly_bind_interval(self) -> None:
        stream = replay._programme()["query_streams"][0]
        capture = _capture(stream, partition=("20260101", "20261231"))
        capture["effective_search"] = stream["search"]
    ...
    def test_full_unpartitioned_programme_can_be_mechanically_complete(self) -> None:
        ...
        def projector(*, query_id: str, **_: object) -> dict:
            return _projection(mapping[query_id])
    ...
    def test_minimization_violation_fails_closed(self) -> None:
        stream = replay._programme()["query_streams"][0]
        bad = _projection()
    ...
    def test_writer_is_deterministic_and_never_emits_raw_or_patient_payloads(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertFalse(manifest["patient_level_fields_included"])
            self.assertFalse(manifest["mdr_text_narratives_included"])
