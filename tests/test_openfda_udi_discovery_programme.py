...
class OpenFdaUdiProgrammeTests(unittest.TestCase):
    ...
    def test_current_programme_validates_offline(self):
        r=validate_programme(self.programme,self.registry);self.assertEqual(r["query_stream_count"],5);self.assertEqual(r["integration_state"],"AVAILABLE");self.assertFalse(r["network_requests_performed"]);self.assertFalse(r["marketing_authorization_claim_created"]);self.assertFalse(r["current_availability_claim_created"])
    ...
    def test_pending_merge_state_now_fails_closed(self):
    ...
    def test_schema_records_available_only(self):
        s=json.loads(Path("schemas/openfda-udi-discovery-programme.schema.json").read_text(encoding="utf-8"));self.assertEqual(s["properties"]["workbench_dependency"]["properties"]["integration_state"]["const"],"AVAILABLE")
