...
class OperationalHealthTests(unittest.TestCase):
    ...
    def test_exact_current_state_is_healthy_with_clean_complete_run(self) -> None:
        ...
        self.assertEqual(report["accountability"]["lifecycle_resolved_source_ids"], ["SRC-PR-015"])
    ...
    def test_typed_active_source_failures_and_retries_degrade_sources_not_engineering(self) -> None:
        report = evaluate_health(
            accountability=self.accountability,
            development_registry=self.registry,
            plan=self.plan,
