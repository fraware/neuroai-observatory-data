...
class ClinicalTrialsTransportDiagnosticContractTests(unittest.TestCase):
    def test_script_is_single_target_body_free_and_non_mutating(self) -> None:
        ...
        self.assertIn('"urllib3_managed_context_pinned_http1_1"', text)
        self.assertIn('"urllib3_stdlib_context_pinned_http1_1"', text)
        self.assertIn('pool_kwargs["ssl_context"] = ssl.create_default_context()', text)
