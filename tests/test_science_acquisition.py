...
m = _load_module(
    ...
    ROOT / "scripts" / "acquire_science_candidates.py",
)
compiler = _load_module(
    "compile_science_queries_for_acquisition_tests",
    ROOT / "scripts" / "compile_science_queries.py",
)
science_validator = _load_module(
    "validate_science_graph_for_acquisition_tests",
    ROOT / "scripts" / "validate_science_graph.py",
)
coverage_validator = _load_module(
    "validate_source_universes_for_acquisition_tests",
    ROOT / "scripts" / "validate_source_universes.py",
)
...
PREDECESSOR_COMPILATION = json.loads(
...
def frozen_plan():
    return compiler.compile_plan(
        copy.deepcopy(PROTOCOL), copy.deepcopy(COMPILATION)
    )


def predecessor_plan():
    return compiler.compile_plan(
        copy.deepcopy(PROTOCOL), copy.deepcopy(PREDECESSOR_COMPILATION)
    )
...
def provider_unit(provider):
    plan = frozen_plan()
    unit = copy.deepcopy(
        next(row for row in plan["query_units"] if row["provider"] == provider)
    )
...
class ScienceAcquisitionTests(unittest.TestCase):
    ...
    def test_current_plan_identity_is_accepted(self):
        plan = frozen_plan()
        self.assertEqual(
            plan["plan_sha256"],
            "a9b8b8999861882c4bc78b27f40f48e476f7cafbbb347b00a0a6cd897406db56",
        )
        self.assertTrue(m.validate_plan_integrity(plan))

    def test_predecessor_plan_is_reproducible_but_not_acquisition_eligible(self):
    ...
    def test_crossref_complete_two_page_query_unit(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                result["coverage_state"], "ISSUED_COMPLETE_QUERY_UNIT"
            )
            self.assertEqual(result["coverage"]["rates"]["discovery"], 1.0)
            self.assertEqual(
                result["freeze"]["retrieval_cutoff"], "2026-08-20T00:00:00Z"
            )
            candidates = list(m._load_jsonl(root / result["candidates_path"]))
            self.assertEqual(
                [row["identifiers"]["doi"] for row in candidates],
                ["10.1234/a", "10.1234/b"],
            )
            ...
            self.assertLess(
                result["page_manifest"][0]["requested_at"],
                result["page_manifest"][0]["observed_at"],
            )
    ...
    def test_europe_pmc_identity_is_source_aware_and_schema_safe(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                candidate["candidate_id"].startswith("SCI-CAND-EUROPE-PMC-")
            )
    ...
    def test_europe_pmc_same_bare_id_different_sources_are_distinct_provider_records(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                {
                    (row["provider_record_source"], row["provider_record_id"])
    ...
    def test_europe_pmc_missing_source_fails_closed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "provider source database code", result["freeze"]["failure_reason"]
            )
    ...
    def test_cursor_stall_is_partial_and_coverage_is_not_issued(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ...
                "CURSOR_DID_NOT_ADVANCE_BEFORE_PROVIDER_TOTAL",
            )
            self.assertIsNone(result["coverage"])
            self.assertEqual(
                result["coverage_state"], "NOT_ISSUED_INCOMPLETE_QUERY_UNIT"
            )
            self.assert_generated_contracts_validate(result, root)
    ...
    def test_scoped_plan_cannot_claim_full_plan_completion(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                manifest["status"], "PARTIAL_OR_SCOPED_ACQUISITION"
            )
    ...
    def test_exact_doi_dedup_is_candidate_only(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                doi_groups[0]["normalized_identifier"], "10.1/shared"
            )
    ...
    def test_complete_result_tampering_blocks_resume_before_network(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            transport = FakeTransport([])
            with self.assertRaisesRegex(
                ValueError, "candidate file digest mismatch"
            ):
                m.acquire_plan(
                    plan,
                    output_root=root,
    ...
    def test_incomplete_attempt_is_archived_before_clean_retry(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                (root / "runs" / first["run_id"] / "run-manifest.json").is_file()
            )
