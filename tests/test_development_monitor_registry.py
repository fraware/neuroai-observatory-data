...
class DevelopmentMonitorRegistryTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.registry = build_development_registry(cls.inputs)
    ...
    def test_exact_registry_view_counts_and_status(self) -> None:
        ...
        self.assertEqual(metadata["record_count"], 227)
        self.assertEqual(metadata["predecessor_record_count"], 224)
        self.assertEqual(metadata["extension_record_count"], 3)
    ...
    def test_only_exact_three_recurring_candidates_are_appended(self) -> None:
        extension = self.registry["sources"][224:]
        self.assertEqual(
            {record["source_id"] for record in extension},
            {"SRC-PR-002", "SRC-PR-007", "SRC-PR-015"},
        )
        self.assertTrue(
        ...
        self.assertNotIn("SRC-PR-010", {record["source_id"] for record in extension})

    def test_monitor_and_source_bindings_are_unique(self) -> None:
        monitors = self.registry["sources"]
        self.assertEqual(len({record["monitor_id"] for record in monitors}), 227)
        self.assertEqual(len({record["source_id"] for record in monitors}), 227)

    def test_registry_view_is_deterministic(self) -> None:
        again = build_development_registry(self.inputs)
