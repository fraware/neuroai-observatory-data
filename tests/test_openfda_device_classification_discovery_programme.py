...
class DeviceClassificationProgrammeTests(unittest.TestCase):
    ...
    def test_current_programme_validates(self):
        r=validate_programme(self.programme,self.registry);self.assertEqual(r["integration_state"],"NOT_IMPLEMENTED");self.assertEqual(r["query_stream_count"],5);self.assertFalse(r["exact_device_identity_claim_created"]);self.assertFalse(r["authorization_claim_created"])
    ...
    def test_schema_keeps_workbench_unimplemented(self):
        s=json.loads(Path("schemas/openfda-device-classification-discovery-programme.schema.json").read_text(encoding="utf-8"));self.assertEqual(s["properties"]["workbench_dependency"]["properties"]["integration_state"]["const"],"NOT_IMPLEMENTED")
