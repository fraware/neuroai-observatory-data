...
spec = importlib.util.spec_from_file_location("partition_proof_current", MODULE_PATH)
...
def _replay(*, first_total: int = 1200, second_total: int = 900, duplicate: bool = False) -> dict:
    normalized = [
        {"docdb_publication_reference": "DOCDB:EP:111:A1", "leaf_query_ids": ["leaf-old"]},
        {"docdb_publication_reference": "DOCDB:EP:222:A1", "leaf_query_ids": ["leaf-recent"]},
    ]
    if duplicate:
        normalized.append({"docdb_publication_reference": "DOCDB:EP:333:A1", "leaf_query_ids": ["leaf-old", "leaf-recent"]})
    return {
        "manifest_sha256": MANIFEST_SHA,
        "controlled_source_id_set_sha256": SOURCE_SHA,
        "input_provenance": {"captured_at": CAPTURED_AT, "controlled_source_id_set_sha256": SOURCE_SHA},
        "query_reports": [
            _leaf("leaf-old", "18000101", "20251231", first_total),
            _leaf("leaf-recent", "20260101", "20260902", second_total),
        ],
...
def _proof() -> dict:
    return {
        ...
        "partition_roots": [{
            "query_id": "DISCOVERY-OPS-BCI-001",
            "applicant_term": None,
            "bounded_parent_cql": f'({ROOT_CQL}) and pd within "18000101 20260902"',
            "parent_probe_pages": [{"xml": "<synthetic-parent />"}],
            "leaf_intervals": [
                {"leaf_query_id": "leaf-old", "lower_date": "18000101", "upper_date": "20251231"},
                {"leaf_query_id": "leaf-recent", "lower_date": "20260101", "upper_date": "20260902"},
            ],
        }],
    }
...
def _parent_projection(total: int = 2100) -> dict:
    return {
        ...
        "coverage": {
            ...
            "candidate_emission_refused_due_to_over_limit": True,
        },
    }
...
class EpoOpsPatentPartitionProofTests(unittest.TestCase):
    def test_exact_dated_partition_reconciliation_can_complete(self) -> None:
        ...
        self.assertEqual(result["controlled_source_id_set_sha256"], SOURCE_SHA)
    ...
    def test_gap_in_child_intervals_blocks_reconciliation(self) -> None:
        ...
        result = proofmod.build_partition_proof(_replay(), proof, projector=lambda **_: _parent_projection())
    ...
    def test_child_denominator_sum_must_equal_parent(self) -> None:
        result = proofmod.build_partition_proof(_replay(first_total=1200, second_total=899), _proof(), projector=lambda **_: _parent_projection(2100))
        self.assertFalse(result["dated_partition_reconciliation_complete"])
    ...
    def test_cross_partition_docdb_duplicate_blocks_reconciliation(self) -> None:
        result = proofmod.build_partition_proof(_replay(duplicate=True), _proof(), projector=lambda **_: _parent_projection())
        self.assertFalse(result["dated_partition_reconciliation_complete"])
    ...
    def test_parent_must_be_over_ops_retrieval_limit(self) -> None:
        result = proofmod.build_partition_proof(_replay(first_total=1000, second_total=900), _proof(), projector=lambda **_: _parent_projection(1900))
        self.assertFalse(result["dated_partition_reconciliation_complete"])
        self.assertIn("PARENT_NOT_OVER_RETRIEVAL_LIMIT", result["root_reports"][0]["blockers"])

    def test_parent_must_fail_closed_without_candidate_emission(self) -> None:
        parent = _parent_projection()
        parent["coverage"]["candidate_emission_refused_due_to_over_limit"] = False
        result = proofmod.build_partition_proof(_replay(), _proof(), projector=lambda **_: parent)
        self.assertIn("PARENT_DID_NOT_FAIL_CLOSED_OVER_LIMIT", result["root_reports"][0]["blockers"])
    ...
    def test_replay_must_be_full_and_not_predeclared_complete(self) -> None:
        replay = _replay()
        replay["reconciliation"]["scope"] = "PARTIAL_VALIDATION"
        replay["reconciliation"]["mechanically_complete"] = True
        ...
        self.assertIn("REPLAY_IMPROPERLY_MARKED_COMPLETE", result["replay_level_blockers"])

    def test_duplicate_partition_root_fails_closed(self) -> None:
        proof = _proof()
        proof["partition_roots"].append(copy.deepcopy(proof["partition_roots"][0]))
        with self.assertRaisesRegex(ValueError, "Duplicate partition proof root"):
            proofmod.build_partition_proof(_replay(), proof, projector=lambda **_: _parent_projection())

    def test_leaf_must_belong_to_same_logical_root(self) -> None:
        replay = _replay()
        replay["query_reports"][0]["query_id"] = "DISCOVERY-OPS-NEURAL-DECODING-AI-001"
        result = proofmod.build_partition_proof(replay, _proof(), projector=lambda **_: _parent_projection())
        self.assertFalse(result["dated_partition_reconciliation_complete"])
        self.assertIn("LEAF_QUERY_ROOT_MISMATCH", result["root_reports"][0]["blockers"])

    def test_unbounded_and_authority_nonclaims_are_preserved(self) -> None:
        result = proofmod.build_partition_proof(_replay(), _proof(), projector=lambda **_: _parent_projection())
        for key in ("unbounded_query_completeness_claim", "global_neuroai_patent_recall_claim", "patent_family_completeness_claim", "automatic_source_admission", "automatic_patent_family_creation", "automatic_entity_creation", "automatic_product_or_system_relationship_creation", "automatic_capability_claim_creation", "automatic_assessment_mutation", "canonical_successor_ready"):
