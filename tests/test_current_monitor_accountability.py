...
from build_current_monitor_accountability import (
    build_projection,
    verify_expected_current_checkpoint,
)
...
class CurrentMonitorAccountabilityTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        source_ids = {
            str(row["record_id"]) for row in cls.sources if row.get("record_id")
        }
        monitor_ids = {
            str(row["record_id"]) for row in cls.monitors if row.get("record_id")
        }
    ...
    def test_exact_current_checkpoint(self) -> None:
        ...
        self.assertEqual(
            self.projection["current"]["lifecycle_resolved_source_ids"], ["SRC-PR-015"]
        )
    ...
    def test_only_active_recurring_gaps_become_monitor_candidates(self) -> None:
        ...
        self.assertTrue(
            all(record["cadence"] for record in candidate["candidate_records"])
        )
        self.assertTrue(
            all(
    ...
    def test_lifecycle_resolved_source_remains_accounted_without_monitor_candidate(
        self,
    ) -> None:
        ...
        self.assertEqual(
            source["successor_discovery_watch_id"], "DISC-SCIENCE-CAREERS-001"
        )
        self.assertEqual(
        ...
        candidate_ids = {
            record["source_id"]
            for record in self.projection["monitor_extension_candidate"][
                "candidate_records"
            ]
        }
        self.assertNotIn("SRC-PR-015", candidate_ids)

    def test_dsmb_announcement_remains_archival_even_when_title_mentions_trial(
        self,
    ) -> None:
        rows = {row["source_id"]: row for row in self.projection["current"]["sources"]}
        self.assertEqual(
            rows["SRC-PR-010"]["accountability_state"], "EXEMPT_WITH_RATIONALE"
        )
    ...
    def test_candidate_reaches_zero_gap_without_rewriting_predecessor(self) -> None:
        ...
        self.assertEqual(
            self.projection["monitor_extension_candidate"]["predecessor_monitor_count"],
            224,
        )
    ...
    def test_projection_is_deterministic(self) -> None:
        ...
        self.assertEqual(
            self.projection["projection_sha256"], again["projection_sha256"]
        )
    ...
    def test_lifecycle_transition_is_not_silently_treated_as_gap(self) -> None:
        rows = [
            row
            for row in self.projection["current"]["sources"]
            if row["accountability_state"] != "MONITORED"
        ]
        archival = [
            row
            for row in rows
            if row["accountability_state"] == "EXEMPT_WITH_RATIONALE"
        ]
        manual = [row for row in rows if row["accountability_state"] == "MANUAL_ONLY"]
        lifecycle = [
            row
            for row in rows
            if row["accountability_state"] == "LIFECYCLE_RESOLVED_ARCHIVAL"
        ]
        ...
        self.assertTrue(
            all(row["rationale"] for row in archival + manual + lifecycle + gaps)
        )
